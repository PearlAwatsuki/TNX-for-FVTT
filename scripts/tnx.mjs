...
async function setupDefaultCardPilesForCast(actor) {
    ...
    try {
        ...
        const handPileName = game.i18n.format("TNX.DefaultHandPileName", {actorName: actor.name});
        ...
        const trumpPileName = game.i18n.format("TNX.DefaultTrumpPileName", {actorName: actor.name});
        ...
        if (diff.name) {
            ...
            try {
                ...
                if (actor.system.handPileId) {
                    const handPile = await fromUuid(actor.system.handPileId);
                    if (handPile) {
                        const newHandName = game.i18n.format("TNX.DefaultHandPileName", { actorName: actor.name });
                ...
                if (actor.system.trumpCardPileId) {
                    const trumpPile = await fromUuid(actor.system.trumpCardPileId);
                    if (trumpPile) {
                        const newTrumpName = game.i18n.format("TNX.DefaultTrumpPileName", { actorName: actor.name });
