        ...
        return {
            actions: new fields.ArrayField(
                new fields.SchemaField({
                    ...
                    // ─── 即死・社会戦タイプ(17-3・神業専用) ───
                    // 即死: 系統(肉体=《死の舞踏》《とどめの一撃》／精神=《神の御言葉》／トループの壊滅=
                    // 《天変地異》《突破》)。肉体・精神は終端状態(完全死亡/精神崩壊)か任意ダメージ(チャートの
                    // 値)を使用時に選び、対象がトループならその1グループが壊滅する(人数 0)。
                    // 「トループの壊滅」は**トループ級しか対象にならない**系統で、結果は壊滅のみ(選ぶものが
                    // 無いので結果の選択も出ない)。効果文「この効果で、キャストやゲストに直接ダメージを
                    // 与えることはできない」のとおり、キャスト/ゲストの行は効果なしで終わる。
