...
export class TnxSocketHandler {
    ...
    static async _onMiracleSwap(data) {
        if (!await TnxSocketHandler._authorizeDelegation(data)) return;
        const { applyMiracleSwapDelegated } = await import("../flow/miracle-flow.mjs");
    ...
    static async _onRepairApply(data) {
        if (!await TnxSocketHandler._authorizeDelegation(data)) return;
        const { applyRepairDelegated } = await import("../flow/repair-flow.mjs");
    ...
    static async _onModificationApply(data) {
        if (!await TnxSocketHandler._authorizeDelegation(data)) return;
        const { applyModificationDelegated } = await import("../flow/modification-flow.mjs");
    ...
    static async _onInterruptGrant(data) {
        ...
        const { grantInterruptToTargets } = await import("../flow/interrupt-grant.mjs");
    ...
    static async _onSessionTeam(data) {
        ...
        const requester = game.users.get(data?.userId);
        if (!requester) return;
        const ss = await import("../session/session-state.mjs");
    ...
    static async _onInfoDisclose(data) {
        ...
        const { applyInfoDisclosure } = await import("../flow/info-gathering.mjs");
