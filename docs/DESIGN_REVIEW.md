...
- フェーズC(将来): JournalEntryPage をカスタムサブタイプ化すれば、ページ作成ダイアログで
  型ドロップダウン選択が標準機能として実現可能。FUTURE_CONSIDERATIONS.md に記録済み。
- 次サブフェーズ 1-2 では TnxScenarioSheet 本体を ApplicationV2 に移行する。
  現在の `sheetClass` フラグ方式は変更しない(シート本体の V2 化のみ)。
