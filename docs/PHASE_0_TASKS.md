...
関連コードの削除または再設計はフェーズ5（HUD/CSS v13 化）で扱う。
...
| ID | 概要 | 担当フェーズ |
|---|---|---|
| KI-010 | `combatSpeed` の表示・入力 UI が未実装（initiative 式は修正済みだが常に 0） | フェーズ3 |
| KI-011 | v13 で JournalDirectory ApplicationV2 化によりカスタム「作成」メニューが機能しない | フェーズ5 または 6 |
| KI-012 | v13 でホットバー折りたたみ機能自体が削除されたため自動折りたたみが no-op | フェーズ5 |
| KI-013 | TnxHud が v13 の Foundry 標準 UI と画面上で物理的に衝突している | フェーズ5 |
...
いずれも HUD/UI レイヤーの再設計を要するため、フェーズ5（HUD/CSS v13 化）で統合的に対処する。
