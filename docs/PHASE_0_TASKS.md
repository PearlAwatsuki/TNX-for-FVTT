...
**推定難易度**: 低 → **高**（v13 で JournalDirectory が ApplicationV2 化されたため再設計が必要）

**前提条件**
なし（P0-08 / P0-09 と独立。同一ファイルだが対象箇所は異なる）

**現状と保留理由**（2026-05-20 更新）
jQuery → DOM API への書き換えは実施済みで `html.find is not a function` エラーは解消した。
しかし v13 で `JournalDirectory` が `DocumentDirectory` → ApplicationV2 ベースに刷新された結果、
作成ボタンのセレクタが `.create-document` から `data-action="createEntry"` に変わっており、
`querySelector(".create-document")` が `null` を返してガード文で静かに return している。
カスタムメニューの差し込みには v13 の `getHeaderControls` フックへの移行が必要であり、
フェーズ0 のスコープ（既存コードのエラー解消）を超えるため本格対応を別フェーズに先送りする。
...
**※ 補足**: P0-09 では querySelector の v13 API 対応（html[0] → html）を実施したが、
v13 ではホットバー折りたたみ機能自体が Foundry コアから削除されているため、
自動折りたたみ処理は機能しない（no-op となる）。詳細は KNOWN_ISSUES.md KI-012 を参照。
関連コードの削除または再設計は、HUD 構造の ApplicationV2 化はフェーズ1、デザイン刷新はフェーズ5 で扱う。
...
| タスク | 内容 |
|---|---|
| P0-02 | `active-effects-list.hbs` の `data-action` 名を switch 文と一致するよう修正 |
| P0-03 | `styleSkill._onSelectChange` の正規表現から誤った `styleSkill.` プレフィックスを除去 |
| P0-04 | `system.json` の initiative 式を `@system.CS.value` → `@system.combatSpeed.value` に修正 |
| P0-05 | v13 実機起動による警告・エラーの観測（ユーザー実施） |
| P0-06 | `V13_MIGRATION_NOTES.md` を実機観測結果で更新 |
| P0-08 | `renderSettingsConfig` フックを jQuery → DOM API に移行 |
| P0-09 | `renderHotbar` フックの `html[0]` → `html` に修正（エラー解消） |
| P0-01 | `system.json` の `compatibility.verified` を `"13.351"` に更新 |
| +追加 | バッドステータスの個別解除機能実装（statusId ベースの dispatch） |

### 保留タスク

| タスク | 理由 |
|---|---|
| P0-07 | jQuery → DOM 移行によりエラーは解消したが、v13 で `JournalDirectory` が ApplicationV2 化された結果 `.create-document` セレクタが機能せず、カスタム「作成」メニューが表示されない。`getHeaderControls` フックへの移行が必要であり、フェーズ0 スコープ外（詳細: KI-011） |

### フェーズ0 で発覚した新規 KNOWN_ISSUES

| ID | 概要 | 担当フェーズ |
|---|---|---|
| KI-010 | `combatSpeed` の表示・入力 UI が未実装（initiative 式は修正済みだが常に 0） | フェーズ4 |
| KI-011 | v13 で JournalDirectory ApplicationV2 化によりカスタム「作成」メニューが機能しない | フェーズ1 |
| KI-012 | v13 でホットバー折りたたみ機能自体が削除されたため自動折りたたみが no-op | フェーズ1 |
| KI-013 | TnxHud が v13 の Foundry 標準 UI と画面上で物理的に衝突している | フェーズ1 |

### フェーズ0 で達成したこと

- v13 起動時の致命的エラー（`html.find is not a function` 等）をすべて解消
- キャストシートの表示・基本動作（スタイル技能、ActiveEffect CRUD、バッドステータス）を確保
- フェーズ0 着手前から存在していた既知バグ（KI-001, KI-003, KI-004）を修正
- `system.json` の `verified` を実機確認済みの v13 バージョンに更新
- v13 移行に関する観測結果を `V13_MIGRATION_NOTES.md` / `KNOWN_ISSUES.md` に文書化

### フェーズ0 で達成できなかったこと

- ジャーナルディレクトリのカスタム「作成」メニュー（P0-07 保留 → KI-011）
- HUD と Foundry 標準 UI の衝突解消（KI-013）
- ホットバー自動折りたたみの代替実装（KI-012）

KI-011・KI-012・KI-013 は HUD/UI レイヤーの再設計を要するため、フェーズ1(HUD + シナリオジャーナルの ApplicationV2 移行)で統合的に対処する。
