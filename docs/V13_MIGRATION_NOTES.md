...
存在しないため処理は no-op となる。

### 追記（2026-05-24）
フェーズ1-0 にて死コードを削除済み（KI-012 解消）。
- `scripts/tnx.mjs` の `renderHotbar` フックハンドラ(コメント含む)を削除
- `scripts/module/tnx-hud.mjs` の `bottomBar`/`actionBar`/`hotbarCollapseButton` 取得ブロックおよび `if` ブロックを削除
なお、同ファイルの §7 サイドバー連動コード(`#sidebar` 等)は別件であり本項目のスコープ外。
