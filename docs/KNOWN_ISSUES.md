...
設計議論を経ずに改変しないこと」に該当するため、新フェーズ5(判定システムの設計)の設計議論で取り扱う。
...
新フェーズ9(戦闘進行/サブシーン)に先立って修正しておくべき。
...
**フェーズ0 で扱うべきか**: No（シート表示・基本動作には関係しない。新フェーズ1(HUD ApplicationV2 移行)以降で対処）
...
**フェーズ0 で扱うべきか**: No（KI-001 の修正で動作は回復する。設計の統一は新フェーズ2(既存シート ApplicationV2 移行)）
...
**フェーズ0 で扱うべきか**: No
新フェーズ4(未実装シートの追加)で対処する。combatSpeed UI はキャストシートへのタブ追加だが
未実装シート作成フェーズと合わせて整理する。ルール解釈に関わるため、ユーザーの判断が必要。
...
**担当フェーズ**: 新フェーズ1(HUD ApplicationV2 移行)または新フェーズ2(既存シートの
ApplicationV2 移行)と統合して設計する可能性が高い。
...
**担当フェーズ**: 新フェーズ1(HUD ApplicationV2 移行)。
...
HUD/UI 関連で、新フェーズ1(HUD ApplicationV2 移行)で統合的に扱う候補。
...
**担当フェーズ**: 新フェーズ2(既存シートの ApplicationV2 移行)。  
...
*最終更新: 2026-05-23(ロードマップ再設計に伴うフェーズ番号更新、KI-016 ユーザー確認結果追記)*
