...
**担当フェーズ**: フェーズ2(Player Actor → User DataModel 移行)。`handMaxSize` の所有モデル
(cast / player / User)の決定はフェーズ2 の User 移行と連動し、ここで決着する。
...
## KI-011 [実機確認待ち]: ジャーナルディレクトリの「アクトシートを作成」導線が v13 で機能しない
...
コアの仕組み上実現できないことが確定した(ドロップダウン選択は JournalEntryPage レイヤーで初めて可能)。

**対応**: 2026-05-24、フェーズ1-1 にて実装。
...
正確な API(プロパティ名等)が一次情報なしには確認できず、動作保証が困難なため。
`renderJournalDirectory` フックが発火することはコードで確認済みであり、
`[data-action="createEntry"]` の存在は KI-011 の調査時点で確定している。

**該当ファイル・箇所**
- `scripts/tnx.mjs` の `renderJournalDirectory` フックハンドラ

**実機確認待ち事項**:
- GM でジャーナルサイドバーを開き「アクトシートを作成」ボタンが見えること
- そのボタンを押して TnxScenarioSheet で開いた JournalEntry が作成されること

**フェーズ0 で扱うべきか**: No

**担当フェーズ**: フェーズ1-1 で対応済み(実機確認後にクローズ)。
...
## KI-013 [実機確認待ち]: TnxHud が Foundry 標準 UI と衝突している
...
- `css/tnx.css`（HUD 関連の CSS）

**影響範囲**
GM・プレイヤーの UI 操作性。特定の Foundry 標準機能へのアクセスが妨げられている
可能性がある。

**フェーズ1-2 で実施した対処（CSS 座標調整、2026-05-24）**:
`css/tnx.css` の HUD セクションの座標値を調整し、v13 標準 UI との被りを軽減した。
...
| `.hud-main-cards` `right` | `310px` | `360px` | サイドバー(~305px)+シーンコントロール(~50px)を回避 |
| `.hand-area` `max-width` | `914px` | `680px` | 1280px 幅画面で右UI と重ならない範囲(5枚) |

また以下の死にルールを削除した:
- `#tnx-hud.sidebar-collapsed .hud-main-cards` — サイドバー連動は KI-012 で廃止済み
- `#tnx-hud.sidebar-collapsed .hud-top-right` — 同上
- `.hud-bottom-bar.hotbar-is-collapsed` — v13 でホットバー折りたたみ自体が廃止済み

**実機確認待ち事項**:
- 各 HUD エリア（山札・捨て札、ニューロ/シーンカード、手札/切り札）が v13 標準 UI と重ならないこと
- カード操作ボタンが押せること
- 微調整が必要な場合は `css/tnx.css` の座標値を実機で見ながら調整する

**申し送り(フェーズ2 への): UI 領域への組み込みによる理想レイアウト**
- 現在は `position: fixed` の独立レイヤー方式。CSS 座標の手動調整には限界がある。
- 将来的には TnxHud を ApplicationV2 化して v13 の `#ui-right` 等の標準 UI 領域に
  組み込む方が理想（隙間に自動配置される）。これはフェーズ2(V2 化)で対応。
- 左上は将来の TokenActionHUD のデフォルト位置として空けたまま維持。
- 右側の中段（上=ニューロ/シーン、下=山札/捨て札の間）は将来の情報項目エリア予定地。

**修正の難易度**: 高（TnxHud の存在の仕方そのものを再設計する必要がある）

**担当フェーズ**: フェーズ2(ApplicationV2 化)で UI 領域組み込みを実施

**関連項目**: KI-011（ジャーナル「作成」メニュー）、KI-012（ホットバー折りたたみ）も
HUD/UI 関連で、フェーズ1 で統合的に扱った。
...
*最終更新: 2026-05-24(ロードマップ再編に伴うフェーズ番号更新。User 移行フェーズ2 昇格・KI-005/010/011/012/013/017/018/019 担当フェーズ修正)*
