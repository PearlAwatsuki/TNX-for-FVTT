...
フェーズ9（ActiveEffect / 派生値計算パイプライン）完了。実効値計算を一本化し、**v2 ActiveEffect 適用モデル**を確立。トーキョーN◎VA のダメージ＝チャート式効果累積を支える**コンディション（バッドステータス・戦闘不能・負傷）基盤**を実装。発火（行動制限・継続ダメージ・回復）は乗る相手のシステム（フェーズ12/13/15）へ申し送り。実機検証はダメージ判定（フェーズ12）が前提のため未実施。

### Added
- **派生値パイプライン**: `CastDataModel.prepareDerivedData` で能力値/制御値の実効値（`total` / `totalControl`）を一本化、最終値 0clamp。`outfitMod` / `appearanceModifier` を B-2 化（フック書き戻しを廃し派生で都度算出）。KI-021（判定の `equipped` バグでスタイル基本値が脱落）解消。
- **v2 ActiveEffect 適用モデル**: base（編集値）と total（実効値）を分離し、エフェクトを total へネイティブモード（加算/上書き/乗算）で直接適用。値バフ `system.<名前空間>`（ability/control/self/parent/category/skill/handMaxSizeMod）、判定バフ `check.` / `controlCheck.`（技能識別キー・能力値・制御）、条件付き `[path op value]`。**同一効果の重複適用不可**（最大採用）＋「重複可」チェックボックス（詳細タブ）。
- **判定バフの可視化**: 判定結果チャットに「判定ボーナス」と寄与エフェクトの内訳（開閉式）。カード・報酬点プレビューにも反映。
- **コンディション（BS・戦闘不能・負傷）**: 全状態を `CONDITION_KINDS` に統合（バッドステータス・戦闘不能タグ・肉体/精神/社会ダメージ）。効果値の固定/可変を区別し、詳細タブに BS ごとの効果値フィールド。判定・派生への適用（酩酊＝達成値/制御値減、衰弱＝制御値減、重圧＝該当能力値の判定遮断、電子妨害＝所持物計算）。`CONFIG.statusEffects` を統合レジストリから生成。
- **ダメージチャート**: 肉体/精神/社会 ×0〜21 段の負傷名・付与条件を同梱（§4.1 境界＝効果文のみユーザー入力）。**ダメージチャート効果文の設定アプリ**（系統タブ×1〜21・ワールド設定）。`applyDamageChartResult`（ダメージ値→負傷状態の付与入口）。狼狽 BS 追加。
- **状態カスケード**: 状態の付与で `inflicts` の別状態を自動付与（status のみ＝コンディション）。**ダメージ由来 AE は効果リスト非表示**（供給元が浮くため。技能由来 BS は通常表示）。
- **カードドロー（衰弱/重圧）**: 効果値をカードで決定（BS受付チャット→山札→スート＝対象能力値・N◎VA値＝数字、ジョーカーはダイアログで引き直し/ワイルドカード）。ドロー主体は衰弱＝RL・重圧＝受けたキャラ。
- **controlNegate**: 制御判定成功で無効/降格。
- **KI-020 解消**: 手札上限への AE 修正（cast の `handMaxSizeMod` 着地＋3層解決）。
- ActiveEffect 設定の status 選択をグループ化（BS/戦闘不能/肉体/精神/社会）。

### Changed
- `CONFIG.ActiveEffect.legacyTransferral = false`（アイテム自己適用・転送の修正）。効果列挙を `allApplicableEffects()` へ。
- アウトフィット分類にコードキー＋表示値を導入（`category.melee` 等で指定可能に）。

### Removed
- **`effectMod` 着地フィールドを全廃**（v2: バフは total へ直接適用）。modeValue/defence/attack/能力値の effectMod 系、旧モードB の死蔵ヘルパーを撤去。

### Fixed
- 衰弱のスート手動選択 UI を撤去（対象・数字は引いて決まる）。
- コンディションの kind を `statuses` から判定（効果値フィールドが出ない不具合）。1 つの AE が複数 BS を持つ場合に対応。
- status 選択のグループ化が v13 `<multi-select>` を壊し「ベタ打ち」化する不具合を修正（Foundry ファクトリで安全に再実装・フォールバック付き）。

### 申し送り
- 発火（恐慌＝リアクション不可・気絶＝メインプロセス不可・捕縛＝武器攻撃不可・狼狽＝ムーブ/メジャー・萎縮憎悪・邪毒の継続ダメージ・回復タイミング）は乗る相手のシステム前提のため 12/13/15。ダメージ算出本体（カード＋攻撃力−軽減）はフェーズ12。
