...
**フェーズ2(Player → User flag 移行)完了(v0.3.1 / 2026-06-05)**
**次はフェーズ3: 既存シートの ApplicationV2 移行(構造のみ)**

フェーズ2 完了内容:
- player Actor が担っていたEXP、履歴、手札の管理を User フラグへ完全移行
- レコードシート（ApplicationV2）の実装
- player Actor の完全廃止
- handMaxSize の権威を User flag に一本化し、`resolveEffectiveHandMaxSize(user)` を新設
  (ゲーム設定 `defaultHandMaxSize` との連携を確立。KI-005 ベース連携決着)

**重要**: TnxHud / TnxScenarioSheet 等の ApplicationV2 化はフェーズ3 で対応する。
...
- フェーズ2: Player Actor → User DataModel 移行(player 廃止) ← 完了 (v0.3.1)
  プレイヤーの EXP・履歴・手札所有を Player Actor から User へ移行。player Actor を廃止。
  cast シートはロジック(EXP 同期・User 参照)のみ User 対応に張り替える(構造 V2 化はフェーズ3)。
  KI-005 ベース連携(handMaxSize 所有モデル確定 + ゲーム設定連携)はここで決着。
  ActiveEffect 修正(KI-020)は新設 ActiveEffect フェーズへ。
- **フェーズ3: 既存シートの ApplicationV2 移行(構造のみ。cast 含む)** ← 現在(次フェーズ)
- フェーズ4: 既存シートの内容追加・改修(不足機能の実装。KI-010 combatSpeed UI 等)
- フェーズ5: CSS リファクタ + デザイン刷新(サイバーパンク基調。HUD デザイン含む)
- フェーズ6: 未実装シートの追加(outfit 系 11 種 + guest/troop/extra の Actor 3 種)
- ActiveEffect フェーズ(新設・位置は今後確定): 派生値計算パイプライン設計 + handMaxSize 修正
  (KI-020)を含む ActiveEffect 連携の実装。フェーズ6 で対象フィールドが固まった後、
  判定システム設計の前を想定。
- フェーズ7: 判定システムの設計
- フェーズ8: 判定システムの実装
- フェーズ9: 神業システムの設計
- フェーズ10: 神業システムの実装
- フェーズ11: 戦闘進行(コンバットトラッカー改修)/ サブシーン的仕組み
- フェーズ12: 公開準備 → v1.0.0

**設計方針(ロードマップ運用)**:
- 「最小実装」の語は使わない。各機能は最低限ではなく作り込む。リスク管理のため
  各フェーズはサブフェーズに細分して着実に仕上げる。
- 判定(フェーズ7・8)と神業(フェーズ9・10)は「設計フェーズ → 実装フェーズ」に分割。
  設計フェーズはチャット(Opus)でのルール言語化・データ構造設計が中心、
  実装フェーズが Claude Code 中心。
- UI フェーズの順序の意図: 所有モデル(User)を先に確立(2)してから上物のシートを作ることで
...
- DataModel(TypeDataModel)ベース。全 24 type(Actor 4 + Item 17 + Card 3)が移行済み。
- Actor types: cast, guest, troop, extra(player は廃止済み)
...
- 神業はゲーム進行に踏み込んで影響する処理が多く、システム側で機能を作り込む必要がある。
  設計はフェーズ9(神業システムの設計)で独立して扱う。
- プレイヤー単位の経験点管理は FEAR 系の重要な特徴。現状の player Actor 実装の設計を
  尊重し、安易な改変はしない。※所有モデルの User 移行はフェーズ2 で計画的に実施。
- FVTT 標準の Scene 切り替えに依存せず、軽量にシーンを切り替えられるサブシーン的な
  仕組みをフェーズ11(戦闘進行/サブシーン)で設計・実装する。
...
- `docs/PHASE_5_TASKS.md`: フェーズ5(CSS リファクタ + デザイン刷新)の作業計画
