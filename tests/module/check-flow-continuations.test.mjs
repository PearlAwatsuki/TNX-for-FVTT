...
// 表は 2026-09-07 に check-continuations.mjs へ移設した(フローの状態を一切参照しないため)
const registry = readFileSync(join(root, "scripts/flow/check-continuations.mjs"), "utf8");

/** 表のキーを宣言順で返す。 */
const keys = [...registry.matchAll(/^ {4}(\w+): \{/gm)].map(m => m[1]);
...
describe("完了継続は CONTINUATIONS が唯一の正本", () => {
  ...
  it("_execute は種別ごとの分岐を持たない(表を回して適用する)", () => {
    ...
    expect(src).toContain("for (const [key, cont] of Object.entries(CONTINUATIONS))");
  ...
  it("スナップショット保存・再実行も表から導く", () => {
    ...
    expect(src).toContain("for (const k of Object.keys(CONTINUATIONS))");
    // _rerunContinuation: 表から種別とポリシーを引く
    expect(src).toContain("Object.keys(CONTINUATIONS).find(k => cc[k])");
  });
